    ...
    struct ShaderData {
        ...
        ~ShaderData()
        {
            ...
            if (mode == MODE_WRITE) // write
            {
                ...
                AiMsgDebug("[luma.partioCacherSampler] writing %d points to \"%s\"", static_cast<int>(num_particles),
                           file.c_str());
            ...
            } else if (mode == MODE_READ) /// read
            {
                ...
                AiMsgInfo("[luma.partioCacherSampler] secondary samples: %d total, %d missed (%.02f%%)",
                          total_secondary,
                          missed_secondary,
        ...
        void update(AtNode* _node)
        {
            ...
            if (mode == MODE_WRITE) {
                ...
                uint AASamples = static_cast<unsigned int>(std::max(AiNodeGetInt(AiUniverseGetOptions(), "AA_samples"),
                                                                    0));
                ...
                if (readPoints) {
                    ...
                    if (!readPoints->attributeInfo(color_channel, colorAttr)) {
                        AiMsgError("[luma.partioCacherSampler] Input cache does not have a %s attribute!",
                                   color_channel);
                        readPoints->release();
                        readPoints = 0;
...
shader_evaluate
{
    ...
    else if (data->readPoints != 0) /// READ MODE
    {
        ...
        data->readPoints->findNPoints(inputPoint, firstSearchPoints, firstSearchDistance, indexes,
                                      pointDistanceSquared);

        // if no points within the first search distance /count
        if (indexes.empty()) {
            if (data->blendMode == BLEND_ERROR) {
                data->readPoints->findNPoints(inputPoint, data->searchPoints, maxSearchDistance, indexes,
                                              pointDistanceSquared);
            }
