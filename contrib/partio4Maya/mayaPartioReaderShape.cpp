...
//MStatus CPartioReaderCache::GetPointPlugValue(MPlug plug, float3 & value)
//{
...
CPartioReaderCache* mayaPartioReaderShape::particleCache() {
	...
	int tmpDecimation = fGeometry.decimation;
	int tmpMode = fGeometry.mode;
	float tmpSphereSize = fGeometry.sphereSize;
	int tmpTesselation = fGeometry.sphereTesselatation;
	MString tmpFilename = fGeometry.cacheFile;

	MObject this_object = thisMObject();
	MPlug plug(this_object, s_particleCache);
	plug.getValue(fGeometry.cacheFile);
	...
	plug.setAttribute(s_sphereRadius);
	plug.getValue(fGeometry.sphereSize);

	plug.setAttribute(s_sphereTesselatation);
	plug.getValue(fGeometry.sphereTesselatation);

	plug.setAttribute(s_sphereRadius);
	plug.getValue(fGeometry.sphereSize);

	plug.setAttribute(s_decimation);
	plug.getValue(fGeometry.decimation);

	float frame = fGeometry.frame;
	float fps = fGeometry.fps;

	MString filename = getFrameFile(fGeometry.cacheFile, int(frame), (fps));

	fGeometry.cacheFile = filename;

	if (filename != tmpFilename) {
		MStatus load = GetPointsFromCache();
	...
	if (fGeometry.decimation != tmpDecimation) {
		fGeometry.updateView = true;
	}

	if (fGeometry.sphereTesselatation != tmpTesselation) {
		fGeometry.updateView = true;
	}

	if (fGeometry.mode == 2 && fGeometry.sphereSize != tmpSphereSize) {
		fGeometry.updateView = true;
	}
